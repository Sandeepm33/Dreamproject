...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      const res = await api.resetPassword(token, { password: form.password });
      if (res.success) {
        setSuccess(true);
      }
    } catch (err: any) {
        ...
        <div className="auth-card-container">
          <div className="auth-visual-side">
            ...
            <div className="visual-message">
              <h2 className="welcome-text">{t('setNewPassword')}</h2>
              <p className="sub-text">{t('chooseStrongPassword')}</p>
            </div>
          ...
          <div className="auth-form-side shadow-2xl">
            <div className="form-inner">
              <div className="form-header">
                ...
                <p>{t('updateAccountCredentials')}</p>
              </div>

              {success ? (
                <div className="success-box">
                  ✅ {t('passwordResetSuccess')}
                  <p style={{ marginTop: 10, color: '#6b7280', fontSize: '14px' }}>{t('loginWithNewPassword')}</p>
                  <button onClick={() => router.push('/login')} className="v-btn-primary" style={{ marginTop: 25 }}>
                    {t('proceedToLogin')}
                  </button>
                </div>
              ) : (
